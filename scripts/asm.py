...
def assemble(lines):
    # pass 1: compute label addresses (word-indexed *4 = byte offset)
    labels = {}
    cleaned = []
    addr = 0
    for raw in lines:
    ...
    maxaddr = words[-1][0] if words else 0
    n = maxaddr//4 + 1
    mem = [0]*n
    for a, w in words:
        mem[a//4] = w & 0xFFFFFFFF
    return mem, labels
    ...
    if args.listing:
        with open(args.listing, "w") as f:
            for i, w in enumerate(mem):
                f.write(f"{i*4:08x} (0x00400000+{i*4:04x}): {w:08x}\n")
            f.write("\nLabels:\n")
